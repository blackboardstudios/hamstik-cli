...
pub(super) async fn bulk(session: &mut Session<'_>, args: &WorkBulkArgs) -> Result<(), CliError> {
    ...
    if let WorkBulkCommand::Run(run_args) = &args.command {
        return bulk_run::run(session, run_args).await;
    }
    ...
    if session.global.dry_run {
        let (operation, method, path, path_template, body): (&str, &str, String, &str, Value) =
            match &args.command {
                ...
                WorkBulkCommand::FromCsv { .. } | WorkBulkCommand::Run(_) => {
                    unreachable!("from-csv and run are handled before any request")
                }
            };
        let count = body
            .get("operations")
