...
One-invocation daily attention view: open Work assigned to you, overdue Work assigned to you, and recent Project activity, composed from existing Public API v1 reads. A failed section is reported and never aborts the others
...
Maximum recent Project activity events to include (0 disables the section; the Project feed is the only activity source Public API v1 exposes)

Default: `10`

### `--since`

Only activity strictly after this time. Accepts RFC 3339, `YYYY\-MM\-DD`, `today`/`yesterday`/`tomorrow`, or a relative offset such as `7d`, `2w`, or `+3h`; input without a UTC offset is read in the host's local time zone
